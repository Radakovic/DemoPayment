...
class InvoiceFactory extends AbstractFactory
{
    ...
    public function __invoke(
        ...
        $order = $order ?? $this->createEntityIfNotExists(MerchantOrder::class);

        $status = $status === InvoiceStatusEnum::PENDING ? InvoiceStatusEnum::SUCCESSFUL : $status;
        ...
        $request = $request ?? $this->createJsonRequest($order, $notificationUrl);
        $response = $response ?? $this->createJsonResponse($order, $invoice);
    ...
    private function createJsonResponse(MerchantOrder $order, Invoice $invoice): string
    {
        $response = [
            'statusCode' => 201,
            'body' => [
                "redirect_url" => $this->faker->url(),
                "deposit_id" => $this->faker->randomNumber(9),
                "user_id" => $this->faker->uuid(),
                "merchant_order_id" => $order->getId()->toString(),
                "payment_info" => [
                    "payment_method" => $invoice->getPaymentMethod(),
                    "payment_method_name" => "XX",
                    "amount" => $order->getAmount() / 100,
                    "currency" => $order->getCurrency(),
                    "expiration_date" => (new DateTimeImmutable('+1 day'))->format('Y-m-d H:i:s'),
