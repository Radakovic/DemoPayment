...
readonly class PSPService implements PSPServiceInterface
{
    ...
    public function postInvoice(array $requestBody): array
    {
        ...
        $merchantOrder = $this->merchantOrderRepository->find($requestBody['body']['merchant_order_id']);
        ...
        return [
            ...
            'body' => [
                ...
                "user_id" => "1111222344556",
                "merchant_order_id" => $requestBody['body']['merchant_order_id'],
                "payment_info" => [
                    "payment_method" => $requestBody['body']['payment_method'],
                    "payment_method_name" => "XX",
                    "amount" => $requestBody['body']['amount'],
                    "currency" => $requestBody['body']['currency'],
                    "expiration_date" => (new DateTimeImmutable('+1 day'))->format('Y-m-d H:i:s'),
                    "created_at" => (new DateTimeImmutable())->format('Y-m-d H:i:s'),
                    "metadata" => [
                        "description" => $requestBody['body']['description'],
    ...
    public function postCallback(array $request): array
    {
        ...
        if ($order->getInvoice()?->getExpirationDate() < $currentDate) {
            return [
                'status_code' => 400,
                'error' => [
                    'error_code' => 1006,
